...
for driver_number in data:
    for traj in data[driver_number]:
        for i in range(len(traj)):
            if traj[i][2] > max_latitude:
                np.delete(traj, i)
            elif traj[i][1] > max_longitude:
                np.delete(traj, i)
            elif traj[i][2] < min_latitude:
                np.delete(traj, i)
            elif traj[i][1] < min_longitude:
                np.delete(traj, i)
    ...
    def find_cell(traj):
        x = math.floor((traj[2]-min_latitude) / (latitude_distance / 500.0))
        y = math.floor((traj[1]-min_longitude) / (longitude_distance / 500.0))
        return [x, y]
    ...
    for traj in data[driver_number]:
        ...
        for point in traj:
            ...
            speed = 0
            if past_cell != []:
                speed = calculate_speed(past_cell[0], cell[0], past_cell[1], cell[1], point[5], last_time)
            features = [cell[0], cell[1], get_hour(point[5]), speed]
    ...
    for traj, seek_traj, service_traj, grid in zip(data[driver_number], training_grids_seek, training_grids_service, grid_array):
        avg_seeking, avg_service = average_distances(seek_traj, service_traj)
        longitude_cell, latitude_cell = find_mode_cell(grid)
        feature = [longitude_cell, latitude_cell, avg_seeking, avg_service, len(service_traj)]
        label = driver_number - 1
    ...
    driver_data = [training, labels]

    new_name = "preprocessed_data/prep_data_driver_" + str(driver_number) + ".pickle"
    print(new_name)
